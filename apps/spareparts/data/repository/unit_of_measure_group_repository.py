...
from apps.spareparts.data.models.sparepart import UnitOfMeasureGroup
...
class UnitOfMeasureGroupRepository:
    ...
    async def read_all(self) -> Sequence[UnitOfMeasureGroup] | None:
        q = select(UnitOfMeasureGroup)
        result = await self.db.execute(q)
        return result.scalars().all()

    async def read_by_id(self, id: int) -> UnitOfMeasureGroup:
